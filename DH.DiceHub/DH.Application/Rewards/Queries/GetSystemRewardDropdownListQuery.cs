...
internal class GetSystemRewardDropdownListQueryHandler(IRepository<ChallengeReward> repository) : IRequestHandler<GetSystemRewardDropdownListQuery, List<GetSystemRewardDropdownListQueryModel>>
{
    ...
    {
        return await this.repository.GetWithPropertiesAsync<GetSystemRewardDropdownListQueryModel>(x => !x.IsDeleted, x => new(x.Id, x.Name), cancellationToken);
    }
}