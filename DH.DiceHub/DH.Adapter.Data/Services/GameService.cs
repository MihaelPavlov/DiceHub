...
public class GameService : IGameService
{
    ...
    public async Task<int> CreateGame(Game game, string fileName, string contentType, MemoryStream imageStream, CancellationToken cancellationToken)
    {
        ...
        await this.tenantDbContext.GameInventories
    ...
    public async Task UpdateGame(Game game, string? fileName, string? contentType, MemoryStream? imageStream, CancellationToken cancellationToken)
    {
        ...
        var dbGame = await this.tenantDbContext.Games
            .AsTracking()
    ...
    {
        return await (
            from g in this.tenantDbContext.Games
    ...
    public async Task<List<GetGameListQueryModel>> GetGameListBySearchExpressionAsync(string searchExpression, string userId, CancellationToken cancellationToken)
    {
        return await
                (from g in this.tenantDbContext.Games
                 ...
                 let likes = this.tenantDbContext.GameLikes.Where(x => x.GameId == g.Id).ToList()
    ...
    {
        await this.tenantDbContext.GameReservations.AddAsync(reservation, cancellationToken);

        var inventory = await this.tenantDbContext.GameInventories.AsTracking().FirstOrDefaultAsync(x => x.GameId == reservation.GameId, cancellationToken)
